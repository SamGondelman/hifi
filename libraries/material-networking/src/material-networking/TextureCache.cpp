    ...
    _content(other._content)
{
    if (_width == 0 || _height == 0 ||
        ...
        (_currentlyLoadingResourceType == ResourceType::KTX && _ktxResourceState != KTXResourceState::FINISHED_LOADING_MIPS)) {
        _startedLoading = false;
    } else if (_currentlyLoadingResourceType == ResourceType::KTX) {
        _ktxResourceState = other._ktxResourceState;
    }
}
...
void NetworkTexture::setImage(gpu::TexturePointer texture, int originalWidth, int originalHeight) {
...
void NetworkTexture::makeLocalRequest() {
    ...
    _ktxResourceState = FINISHED_LOADING_MIPS;
