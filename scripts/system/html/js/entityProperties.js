        ...
        properties: [
            {
                ...
                type: "textarea",
                buttons: [ { id: "clear", label: "Clear Procedural Data", className: "red", onClick: clearProceduralData }, 
                           { id: "edit", label: "Edit as JSON", className: "blue", onClick: newJSONProceduralEditor },
                           { id: "save", label: "Save Procedural Data", className: "black", onClick: saveProceduralData } ],
                propertyID: "proceduralData",
            },
        ]
    },
];
...
const GROUPS_PER_TYPE = {
  ...
  Shape: [ 'base', 'shape', 'spatial', 'behavior', 'collision', 'physics', 'procedural' ],
  Text: [ 'base', 'text', 'spatial', 'behavior', 'collision', 'physics' ],
  Zone: [ 'base', 'zone', 'spatial', 'behavior', 'collision', 'physics', 'procedural' ],
...
/**
 * PROCEDURAL DATA FUNCTIONS
 */

function clearProceduralData() {
    let elProceduralData = getPropertyInputElement("proceduralData");
    ...
    updateProperty('proceduralData', elProceduralData.value, false);
}

function newJSONProceduralEditor() {
    deleteJSONProceduralEditor();
...
function setProceduralDataFromEditor(noUpdate) {
    ...
    if (json === null) {
        ...
        let text = proceduralEditor.getText();
        if (noUpdate === true) {
            EventBridge.emitWebEvent(
                JSON.stringify({
                    id: lastEntityID,
            ...
            updateProperty('proceduralData', text, false);
        }
    }
}
...
function createJSONProceduralEditor() {
    let container = document.getElementById("property-proceduralData-editor");
    let options = {
        search: false,
        mode: 'tree',
        modes: ['code', 'tree'],
        ...
        onChange: function() {
            let currentJSONString = proceduralEditor.getText();

            if (currentJSONString === '{"":""}') {
                return;
            }
            $('#property-proceduralData-button-save').attr('disabled', false);
...
function showSaveProceduralDataButton() {
    $('#property-proceduralData-button-save').show();
}

function hideSaveProceduralDataButton() {
    $('#property-proceduralData-button-save').hide();
}

function disableSaveProceduralDataButton() {
    $('#property-proceduralData-button-save').attr('disabled', true);
}

function showNewJSONProceduralEditorButton() {
    $('#property-proceduralData-button-edit').show();
}

function hideNewJSONProceduralEditorButton() {
    $('#property-proceduralData-button-edit').hide();
}

function showProceduralDataTextArea() {
    $('#property-proceduralData').show();
}

function hideProceduralDataTextArea() {
    $('#property-proceduralData').hide();
}

function hideProceduralDataSaved() {
    $('#property-proceduralData-saved').hide();
}

function showStaticProceduralData() {
    if (proceduralEditor !== null) {
        $('#property-proceduralData-static').show();
        $('#property-proceduralData-static').css('height', $('#property-proceduralData-editor').height());
        $('#property-proceduralData-static').text(proceduralEditor.getText());
    }
}

function removeStaticProceduralData() {
    $('#property-proceduralData-static').hide();
}
    ...
    for (i = 0; i < inputs.length; ++i) {
        ...
        field.on('focus', function(e) {
            if (e.target.id === "property-userData-button-edit" || e.target.id === "property-userData-button-clear" || 
                ...
                e.target.id === "property-proceduralData-button-edit" || e.target.id === "property-proceduralData-button-clear") {
                return;
            } else {
                if ($('#property-userData-editor').css('height') !== "0px") {
                    saveUserData();
                ...
                if ($('#property-proceduralData-editor').css('height') !== "0px") {
                    saveProceduralData();
                }
            }
        });
...
function loaded() {
    openEventBridge(function() {
            ...
            EventBridge.scriptEventReceived.connect(function(data) {
                ...
                if (data.type === "server_script_status") {
                    ...
                    if (data.selections.length === 0) {
                        ...
                        deleteJSONProceduralEditor();
                        getPropertyInputElement("proceduralData").value = "";
                        showProceduralDataTextArea();
                        showSaveProceduralDataButton();
                        showNewJSONProceduralEditorButton();
                        ...
                        let proceduralJson = null;
                        try {
                            proceduralJson = JSON.parse(selectedEntityProperties.proceduralData);
                        } catch (e) {
                            // normal text
                            deleteJSONProceduralEditor();
                            getPropertyInputElement("proceduralData").value = selectedEntityProperties.proceduralData;
                            showProceduralDataTextArea();
                            showNewJSONProceduralEditorButton();
                            hideSaveProceduralDataButton();
                            hideProceduralDataSaved();
                        ...
                        if (selectedEntityProperties.locked) {
                            ...
                            disableSaveProceduralDataButton();
                        }
        ...
        // Procedural Data
        let proceduralDataProperty = properties["proceduralData"];
        let elProceduralData = proceduralDataProperty.elInput;
        let proceduralDataElementID = proceduralDataProperty.elementID;
        elDiv = elProceduralData.parentNode;
        let elStaticProceduralData = document.createElement('div');
        elStaticProceduralData.setAttribute("id", proceduralDataElementID + "-static");
        let elProceduralDataEditor = document.createElement('div');
        elProceduralDataEditor.setAttribute("id", proceduralDataElementID + "-editor");
        let elProceduralDataSaved = document.createElement('span');
        elProceduralDataSaved.setAttribute("id", proceduralDataElementID + "-saved");
        elProceduralDataSaved.innerText = "Saved!";
        elDiv.childNodes[JSON_EDITOR_ROW_DIV_INDEX].appendChild(elProceduralDataSaved);
        elDiv.insertBefore(elStaticProceduralData, elProceduralData);
        elDiv.insertBefore(elProceduralDataEditor, elProceduralData);
